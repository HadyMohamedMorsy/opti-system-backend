...
export class SubfooterController implements SelectOptions, RelationOptions {
  constructor(private readonly service: SubfooterService) {}
  ...
  @HttpCode(200)
  @Auth(AuthType.None)
  async getActiveSubfooter() {
    return this.service.getActiveSubfooter();
  }

  @Post("/store")
  @Auth(AuthType.Bearer)
  @Roles(Role.SUPER_ADMIN, Role.SYSTEM_ADMIN, Role.CONTENT_MANAGER)
  async create(@Body() create: CreateSubfooterDto, @Req() req: Request) {
  ...
  @Auth(AuthType.Bearer)
  @Roles(Role.SUPER_ADMIN, Role.SYSTEM_ADMIN, Role.CONTENT_MANAGER)
  async update(@Body() update: UpdateSubfooterDto, @Req() req: Request) {
