...
@Controller("partners")
export class PartnersController implements SelectOptions, RelationOptions {
  ...
  async getActivePartners() {
    return this.service.getActivePartners();
  }

  @Post("/store")
  @Auth(AuthType.Bearer)
  @Roles(Role.SUPER_ADMIN, Role.SYSTEM_ADMIN, Role.CONTENT_MANAGER)
  async create(@Body() create: CreatePartnersDto, @Req() req: Request) {
    const contentWithCreatedBy = create.content.map(item => ({
      language_id: item.language_id,
      title: item.title,
      description: item.description,
      partners_logos: item.partners_logos,
  ...
  @Auth(AuthType.Bearer)
  @Roles(Role.SUPER_ADMIN, Role.SYSTEM_ADMIN, Role.CONTENT_MANAGER)
  async update(@Body() update: UpdatePartnersDto, @Req() req: Request) {
